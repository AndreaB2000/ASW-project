...
export const getMove = async (board: Board): Promise<Move | undefined> => {
    ...
    try {
        ...
        const move = result[0];
        console.log("Move from Prolog:", move);
...
const parseBoard = (Board: Board): string => {
    ...
    const cellList = cells.flatMap((row, rowIndex) =>
        row.flatMap((cell, colIndex) => {
            if (!cell.pile) return [];
            const owner = cell.pile.owner;
            return [`cell(${colIndex}, ${rowIndex}, ${owner === 'bot' ? 'me' : 'opponent'}, ${cell.pile.numberOfGrains})`];
        })
    ).join(", ");
